    ...
    if (taView.exists()) {
        ...
        taView.findOne().parent().children().forEach(function (child) {
            ...
            if (typeof (text) === "string" && text.indexOf(eng_btn_flag) >= 0) {
                //可以收取的能量
                //hisEng = parseInt(text.replace(/\D/g, ""));
                clickPos(new Pos(posb.centerX(), posb.centerY()), 1000);
        ...
        if(!self){
            //taQty();//点击完所有能量后，本周收取TA的结束状态
            totalQty = taQty() - weekQty
        }
       

        if (totalQty == 0) {
            //自己的或者是没有收取到能量(因为如果对方有可以帮忙收取的能量，还是能从逛一逛进入的)
        } else {

            var index = getFEIndexByName(myfriendName);
            if (index >= 0) {
                var qtys = friendEngMap[index].qtys;
                friendEngMap[index].qtys = totalQty;
            }else{
                friendEngMap.push(new FriendEng(myfriendName, totalQty));
            }
            tLog("本次从'"+myfriendName+"' 获取能量 "+totalQty+"g");
        }
...
function getEng() {
    ...
    while (continueFlag) {
        //点击逛一逛
        clickPosR(explorePosR, 250+randomTo(100));//点击一次之后紧接着再点一次,防止界面有文字提示
...
function main() {
    ...
    var summary = "本次能量统计信息：\r\n";
    for (i = 0; i < friendEngMap.length; i++) {
        //console.log("从好友" + friendEngMap[i].name + "那里收取能量：" + friendEngMap[i].qtys + "g");
        summary += friendEngMap[i].name+"："+friendEngMap[i].qtys + "g"+"\r\n";
    }
    summary += "\r\n";
    summary += "本次总计："+thisTimeTotalEng+ "g"+"\r\n";
    console.log(summary);
